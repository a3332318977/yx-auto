    ...
    await Promise.allSettled(urls.map(async (url) => {
        try {
            ...
            lines.forEach(line => {
                ...
                results.add(hostPart.includes(':') ? line : `\( {hostPart}: \){port}${remark}`);
            });
        } catch (e) {}
    }));
...
function generateLinksFromSource(list, user, workerDomain, disableNonTLS = false, customPath = '/', echConfig = null) {
    ...
    list.forEach(item => {
        ...
        portsToGenerate.forEach(({port, tls}) => {
            ...
            const nodeName = `\( {nodeNameBase}- \){port}${tls ? '-WS-TLS' : '-WS'}`;
...
async function generateTrojanLinksFromSource(list, user, workerDomain, disableNonTLS = false, customPath = '/', echConfig = null) {
    ...
    list.forEach(item => {
        ...
        portsToGenerate.forEach(({port, tls}) => {
            ...
            const nodeName = `\( {nodeNameBase}- \){port}${tls ? '-Trojan-WS-TLS' : '-Trojan-WS'}`;
...
async function handleSubscriptionRequest(request, user, customDomain, piu, ipv4Enabled, ipv6Enabled, ispMobile, ispUnicom, ispTelecom, evEnabled, etEnabled, vmEnabled, disableNonTLS, customPath, echConfig = null) {
    ...
    // 【修复2】GitHub优选 / 优选API - 全协议支持
    if (egi) {
        try {
            let allList = [];

            if (piu && piu.toLowerCase().startsWith('https://')) {
                const apiResults = await 请求优选API([piu]);
                allList = apiResults.map(line => {
                    const match = line.match(/^(\[?[\da-fA-F:.]+\]?)(?::(\d+))?(?:#(.+))?$/);
                    return match ? { ip: match[1].replace(/[\[\]]/g,''), port: parseInt(match[2]||'443'), name: match[3]||match[1] } : null;
                }).filter(Boolean);
            } else if (piu && piu.includes('\n')) {
                const 完整列表 = await 整理成数组(piu);
                const apiUrls = 完整列表.filter(l => l.toLowerCase().startsWith('https://'));
                if (apiUrls.length) {
                    const apiResults = await 请求优选API(apiUrls);
                    allList.push(...apiResults.map(line => {
                        const match = line.match(/^(\[?[\da-fA-F:.]+\]?)(?::(\d+))?(?:#(.+))?$/);
                        return match ? { ip: match[1].replace(/[\[\]]/g,''), port: parseInt(match[2]||'443'), name: match[3]||match[1] } : null;
                    }).filter(Boolean));
                }
            } else {
                allList = await fetchAndParseNewIPs(piu);
            }

            if (allList.length > 0) {
                await addNodesFromList(allList);
            }
        } catch (e) {
            console.error('egi失败:', e);
        }
    }

    if (finalLinks.length === 0) {
        finalLinks.push(`vless://00000000-0000-0000-0000-000000000000@127.0.0.1:80?encryption=none&security=none&type=ws&host=error.com&path=%2F#所有节点获取失败`);
    }
...
// Clash / Surge / Quantumult 配置生成函数（保持原样，略微精简）
function generateClashConfig(links) { /* 原函数保持不变 */ }
function generateSurgeConfig(links) { /* 原函数保持不变 */ }
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (pathMatch) {
            ...
            const echEnabled = url.searchParams.get('ech') === 'yes' || (url.searchParams.get('ech') === null && enableECH);
